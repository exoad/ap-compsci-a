...
public class RPSLSGUI extends JPanel
{
  ...
  { 
    ...
    currentMatchCounter = new JLabel("Current match " + currentMatch + " of " + (matches.length - 1), JLabel.CENTER);
    matchesRemaining = new JLabel("Games remaining: " + (NUM_GAMES - currentGamesPlayed), JLabel.CENTER);
    ...
    playerWinProgress.setBackground(Color.BLUE); // trying background and foreground
    ...
    currentMatchCounter.setText("Current match " + currentMatch + " of " + (matches.length - 1));
  ...
  public void outputResults() {
    try {
      ...
      fw.write("\"" + playerA.coderName() + "\",\"" + scoreA + "\",\"" + playerB.coderName() + "\",\"" + scoreB + "\"\n");
      fw.close();
  ...
  private class TimerListener implements ActionListener
  {
    public void actionPerformed(ActionEvent e)
    {
      if (!paused)
      {
        ...
        matchesRemaining.setText("Games remaining: " + (NUM_GAMES - currentGamesPlayed));
        playerWinProgress.setValue((int)((double)scoreA / (2 * currentGamesPlayed) * 100));
        ...
        if (currentGamesPlayed >= NUM_GAMES) 
        {
          t.stop();
          outputResults();
          //update the three progress bars and scores (using labels for score makes this super slow)
          //playerAScore.setText("" + scoreA);
          //playerBScore.setText("" + scoreB);
        }
      }
