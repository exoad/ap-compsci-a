...
if [ "$choiceVAR" == "1" ]; then
  ...
  if [ "$GIT_IS_AVAILABLE" -eq 0 ]; then
    echo -e "\e[32mgit$successAPP_INSTALLED"
    echo -e "\e[0m"
    printf "Fetching...\n"
    if [ ! -d "$repo_FOLDER" ]; then
      echo "Cloning the repository into folder $repo_FOLDER"
      echo
      ...
      echo "Repository folder detected, fetching the latest into the folder $repo_FOLDER"
      echo
      cd "$repo_FOLDER"
      git pull
      exit
