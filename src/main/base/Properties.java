...
public class Properties {
  ...
  protected void checkNull() throws Exception {
    if (dir == null || dir.equals("") || dir.contains(" ") || subDir == null ||
        subDir.equals("") || subDir.contains(" "))
      throw new Exception(
          "File directory is invalid because it cannot be unsolved or was not declare properly");
  }

  protected boolean check() throws Exception {
    return dir == null || dir.equals("") || dir.contains(" ") ||
        subDir == null || subDir.equals("") || subDir.contains(" ");
  }
  ...
  public String readLineNumber(int number, Object subDir) throws Exception {
    if (new File(dir + subDir.toString()).exists()) {
      ...
      try (Stream<String> lines =
               Files.lines(Paths.get(dir + subDir.toString()))) {
        t = lines.skip(number).findFirst().get();
        List<String> f = Files.readAllLines(Paths.get(dir + subDir.toString()),
    ...
    throw new Exception("Failed to retrieve the requested data from " +
                        subDir.toString() + " of line at " + number);
  }
}
